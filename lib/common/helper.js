...
var googleSignIn = function (email, pass, additionalChallengeCityAnswer) {
  ...
  if (browser.isVisible('#identifierId')) {
  ...
  browser.waitUntil(()=>{
    ...
    if (browser.isVisible('#headingText')) {
      browser.waitForVisible("div=Enter the city you usually sign in from");
      browser.element("div=Enter the city you usually sign in from").click();
      browser.waitForVisible("#challengePickerList");
    }
    if (browser.isVisible('#challengePickerList')) {
  ...
  }, 30000, "could not complete signin process");
